...
if (isset($_POST['devadd'])) {
    ...
    $paysl = $_POST['paysl'];
    $mthtavoir = '';
    if (isset($_POST['mthtavoir'])) {
    $mthtavoir = $_POST['mthtavoir'];
    }
    ...
    $avoirProd = count($_POST['avoirProd']);
    for ($key = 0; $key < $avoirProd; $key++) {
        $avoirElt = addslashes($_POST['avoirProd'][$key]);
        $tabAvoir = explode(",", $avoirElt);
        $avoirId = $tabAvoir[0];
        mysqli_query($link, "INSERT INTO  `detail_facture_avoir`(id_facture ,`num`, `id_avoir`) VALUES('" . $maximum . "','" . $num . "','" . $avoirId . "') ");
         
        // disable avoir status
        mysqli_query($link, "UPDATE `avoir` SET etat_avoir = '2' where id='" . $avoirId . "' ");
 }
    ...
    $pourcentageRemise = '';
    if (isset($_POST['tva2'])) {
    $pourcentageRemise = ($_POST['tva2'] * $mtht) / 100;
    $montant_total_ht = $mtht - $pourcentageRemise;
...
//    echo    "INSERT INTO  `facture`(id,`num`, `titre`, `client`,date_add,validite,reg , prix_ht, tva, prix_ttc,mode_liv,frais_liv ,etat,type,adr,cp,pays,ville,cltnom,cltpre ) VALUES(" . $maximum . ",'" . $num . "','" . $titre . "','" . $client . "','" . $dateadd . "','" . $datee . "'," . $reg . ",'" . $mtht . "','" . $mttva . "','" . $mtttc . "','" . $mode_liv . "','" . $frais_liv . "','" . $etat . "','" . $type . "','" . $adrf . "','" . $cpf . "','" . $paysf . "','" . $villef . "','" . $cltnom . "','" . $cltpre . "'";die;
    ...
    mysqli_query($link, "INSERT INTO  `facture`(id,`num`, `titre`, `client`,date_add,validite,reg , prix_ht, tva, prix_ttc,mode_liv,frais_liv,tva_liv,etat,type,adr,cp,pays,ville,cltnom,cltpre,prixAvoir,prixttcremise,netpayer,remise,montant_total_ht,remise_globale,commentaire ) VALUES(" . $maximum . ",'" . $num . "','" . $titre . "','" . $client . "','" . $dateadd . "','" . $datee . "'," . $reg . ",'" . $mtht . "','" . $mttva . "','" . $mtttc . "','" . $mode_liv . "','" . $frais_liv . "','" . $tvaLiv . "','" . $etat . "','" . $type . "','" . $adrf . "','" . $cpf . "','" . $paysf . "','" . $villef . "','" . $cltnom . "','" . $cltpre . "','" . $mthtavoir . "','" . $prixttcremise . "','" . $netapayer . "','" . $remise . "','" . $montant_total_ht . "','" . $pourcentageRemise . "','" . $commentaire . "') ")or die(mysqli_error($link));
    //test num facture
  $idFacture = mysqli_insert_id($link);
  if ($idFacture == 0) {
    $numFacture = "FAC000000";
} else {

    if (($idFacture < 10)) {
        $numFacture = "FAC00000" . $idFacture;
    }
    if (($idFacture < 100) && ($idFacture >= 10)) {
        $numFacture = "FAC0000" . $idFacture;
    }
    if (($idFacture < 1000) && ($idFacture >= 100)) {
        $numFacture = "FAC000" . $idFacture;
    }
    if (($idFacture < 10000) && ($idFacture >= 1000)) {
        $numFacture = "FAC00" . $idFacture;
    }
    if (($idFacture < 100000) && ($idFacture >= 10000)) {
        $numFacture = "FAC0" . $idFacture;
    }
}


mysqli_query($link, "UPDATE `facture` SET `num` = '" . $numFacture . "' WHERE `id` = '" . $idFacture . "' ")or die(mysqli_error($link));
mysqli_query($link, "UPDATE `detail_facture` SET `num` = '" . $numFacture . "' WHERE `id_devis` = '" . $idFacture . "' ");
mysqli_query($link, "UPDATE `detail_facture_avoir` SET `num` = '" . $numFacture . "' WHERE `id_facture` = '" . $idFacture . "' ");
