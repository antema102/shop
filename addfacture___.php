...
<!--                                        </div>-->
        ...
        <script>
            ...
            function avoirClient() {
                $('#modalinfavoir .modal-body p').remove();
                var client = document.getElementById("clientavoire").value;
                if(client) {
                $.ajax({
                    type: "POST",
                    url: "infavoir.php?client=" + client,
                    success: function (data) {
                       // calcul();
                        $("#prdl").append(data);
                        $.ajax({
                            type: "POST",
                            url: "calcul.php?client="+client,
                            success: function (data) {
                                document.getElementById("calcul").innerHTML = data;
                            }
                    });
...
<?php } ?>
            ...
            var countries2 = [
                <?php
                $sql_avoir = mysqli_query($link, "SELECT * FROM `avoir`");
                while (
                $data_avoir = mysqli_fetch_array($sql_avoir)) {
                ?>
                <?php
                $sql_client = mysqli_query($link, "SELECT * FROM `client` where id ='".$data_avoir['client']."'");
                while (
                $data_client = mysqli_fetch_array($sql_client)) {
               //
                ?>
                "<?php echo $data_client['nom'] ?>  <?php echo $data_client['prenom'] ?> ---ID <?php echo $data_client['id'] ?>",
                <?php } ?>
                <?php } ?>
            ];
            /*initiate the autocomplete function on the "devc" element, and pass along the countries array as possible autocomplete values:*/
            autocomplete(document.getElementById("clientavoire"), countries2);
