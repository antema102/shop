...
class SSP
{
     ...
     *  @return string SQL order by clause
     */
    public static function orderClients($request, $columns)
    {
        $order = '';
    ...
    /**
     * Searching / Filtering (Custom filter for clients with support of FULLTEXT)
     *
     * Construct the WHERE clause for server-side processing SQL query.
     *
     * NOTE this does not match the built-in DataTables filtering which does it
     ...
     *  @return string SQL where clause
     */
    public static function filterClients($request, $columns, &$bindings)
    {
        $globalSearch = array();
        $columnSearch = array();
        $dtColumns = self::pluck($columns, 'dt');
        ...
        if (isset($request['search']) && $request['search']['value'] != '') {
            ...
            $str = trim($str);
            // add a star after every word
            $str = preg_replace('/(\w+)/', '$1*', $str);
            // search
            if (strlen($str) >= 2) {
                // ALTER TABLE `client` ADD FULLTEXT(`nom`,`prenom`,`email`,`tel`,`mob`,`nom_soc`)
                $globalSearch[] = "MATCH(`nom`,`prenom`,`email`,`tel`,`mob`,`nom_soc`) AGAINST('" . $str . "' IN BOOLEAN MODE)";
            }
        }
     ...
     *  @return array          Server-side processing response array
     */
    public static function simple2($request, $conn, $table, $primaryKey, $columns)
    {
        $bindings = array();
        $db = self::db($conn);
        ...
        // Build the SQL query string from the request
        $limit = self::limit($request, $columns);
        $order = self::orderClients($request, $columns);
        $where = self::filterClients($request, $columns, $bindings);
     ...
     *  @return array          Server-side processing response array
     */
    public static function complex22($request, $conn, $table, $primaryKey, $columns, $whereResult = null, $whereAll = null)
    {
