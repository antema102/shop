...
<div class="modal fade" id="exampleModal_' . $d . '" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    array('db' => 'id', 'dt' => 1, 'formatter' => function($d, $row) {
        include 'db.php';
        $tab1 = mysqli_fetch_array(mysqli_query($link, "SELECT * FROM `client` WHERE id=" . $d . " and etiquet='2' "));
    ...
    array('db' => 'id', 'dt' => 2, 'formatter' => function($d, $row) {
        include 'db.php';
        $tab = mysqli_fetch_array(mysqli_query($link, "SELECT * FROM `client` WHERE id=" . $d . " and etiquet='2'"));
        if ($tab['etiquet'] == "2") {
...
//} else {
echo json_encode(
    SSP::complex($_GET, $sql_details, $table, $primaryKey, $columns, null, "etiquet = '2'")
);
//}
?>