...
<div class="modal fade" id="exampleModal_' . $d . '" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    array('db' => 'id', 'dt' => 3, 'formatter' => function($d, $row) {
        include 'db.php';
        $tab = mysqli_fetch_array(mysqli_query($link, "SELECT * FROM `produit` WHERE id=" . $d . " and stock = '0' "));
        return
...
// else {
//            if ($tab['stock'] <= ( $tab['stock_min'] - 1 )  ) {
////                return '<i class="fas fa-circle" style="color:red;margin-right:12px;"></i>';
////            }
//            if ($tab['stocka'] == '1' &&  $tab['stock_min']=="0" ) {
//                return '<i class="fas fa-circle" style="color:red ;margin-right:12px;cursor: pointer;"></i>';
//            }
//            if ($tab['stock'] > ( $tab['stock_min'] + 1 )) {
//                return '<i class="fas fa-circle" style="color:#008000;margin-right:12px;"></i>';
//            }
//            if ($tab['stock'] == ( $tab['stock_min'] + 1 ) || ($tab['stock'] == $tab['stock_min'])) {
...
echo json_encode(
    SSP::simple($_GET, $sql_details, $table, $primaryKey, $columns,null, "stock = 0 ")
);
?>