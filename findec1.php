...
//    }),
    array('db' => 'id', 'dt' => 1, 'formatter' => function($d, $row) {
        include 'db.php';
        $tab1 = mysqli_fetch_array(mysqli_query($link, "SELECT * FROM `client` WHERE id=" . $d . " and etiquet='1' "));
    ...
    array('db' => 'id', 'dt' => 2, 'formatter' => function($d, $row) {
        include 'db.php';
        $tab = mysqli_fetch_array(mysqli_query($link, "SELECT * FROM `client` WHERE id=" . $d . " and etiquet='1'"));
        if ($tab['etiquet'] == "1") {
...
//} else {
    echo json_encode(
        SSP::complex($_GET, $sql_details, $table, $primaryKey, $columns, null, "etiquet = '1'")
    );
//}
?>