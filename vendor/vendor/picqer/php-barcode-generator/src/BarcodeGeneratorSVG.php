...
class BarcodeGeneratorSVG extends BarcodeGenerator
{
    ...
    public function getBarcode($barcode, $type, int $widthFactor = 2, int $height = 30, string $foregroundColor = 'black')
    {
        ...
        $svg .= '<svg width="' . $width . '" height="' . ($height+45) . '" viewBox="0 0 ' . $width . ' ' . $height . '" version="1.1" xmlns="http://www.w3.org/2000/svg">' . PHP_EOL;
        ...
        $svg .= "\t".'<text style="font: 20px monospace" text-anchor="middle" x="123" y="120">'.$barcode.'</text>' . PHP_EOL;
        $svg .= "\t</g>" . PHP_EOL;
        $svg .= '</svg>' . PHP_EOL;
